...
import { Member, Session, VisitorRecord, Justification } from '../types/masonic';
...
export function generateJustificationDocumentPDF(
  justification: Justification,
  member?: Member | { fullName: string; cim: string; degree: string },
  session?: Session
) {
  const doc = new jsPDF({
    orientation: 'portrait',
  ...
  doc.setFontSize(14);
  doc.setTextColor(15, 23, 42);
  doc.text("A.R.L.S. FRATERNIDADE DA FRANCA Nº 3571", pageWidth / 2, 22, { align: 'center' });

  doc.setFontSize(9.5);
  doc.setFont('times', 'normal');
  doc.setTextColor(71, 85, 105);
  doc.text("ORIENTE DE FRANCA/SP — JURISDICIONADA AO GOSP", pageWidth / 2, 28, { align: 'center' });

  doc.setLineWidth(0.3);
  doc.setDrawColor(212, 175, 55);
  doc.line(25, 32, 185, 32);

  // Title Box
  doc.setFillColor(248, 250, 252);
  doc.setDrawColor(203, 213, 225);
  doc.roundedRect(marginX, 38, contentWidth, 22, 2, 2, 'FD');

  doc.setFont('times', 'bold');
  doc.setFontSize(12.5);
  doc.setTextColor(180, 130, 20);
  doc.text("COMPROVANTE DE JUSTIFICATIVA REGIMENTAL", pageWidth / 2, 47, { align: 'center' });

  doc.setFont('times', 'normal');
  doc.setFontSize(9);
  doc.setTextColor(30, 41, 59);
  doc.text(`Protocolo: ${justification.id}  |  Categoria: ${justification.category}`, pageWidth / 2, 54, { align: 'center' });
  ...
  doc.setFontSize(10.5);
  doc.setTextColor(15, 23, 42);
  doc.text("DADOS DO OBREIRO E DA SESSÃO", marginX + 6, currentY + 7);

  doc.setFont('times', 'normal');
  doc.setFontSize(9.5);
  doc.setTextColor(51, 65, 85);
  const memName = member?.fullName || 'Obreiro do Quadro';
  const memCim = member?.cim || '---';
  const memDegree = member?.degree || 'Mestre';
  doc.text(`Obreiro: ${memName.toUpperCase()} (CIM: ${memCim})`, marginX + 6, currentY + 14);
  doc.text(`Grau Maçônico: ${memDegree}`, marginX + 6, currentY + 20);

  const sessTitle = session?.title || 'Sessão Maçônica';
  const sessDate = session?.date ? session.date.split('-').reverse().join('/') : 'Data da Sessão';
  doc.text(`Sessão Relacionada: ${sessTitle}`, marginX + 6, currentY + 26);
  doc.text(`Data da Sessão: ${sessDate}  |  Enviado em: ${new Date(justification.submittedAt).toLocaleDateString('pt-BR')}`, marginX + 6, currentY + 32);
  ...
  doc.setFontSize(10.5);
  doc.setTextColor(15, 23, 42);
  doc.text("MOTIVO REGIMENTAL APRESENTADO:", marginX, currentY);

  currentY += 5;
  doc.setFont('times', 'italic');
  doc.setFontSize(9.5);
  doc.setTextColor(30, 41, 59);
  const reasonLines = doc.splitTextToSize(`"${justification.reason}"`, contentWidth);
  doc.text(reasonLines, marginX, currentY);
  currentY += reasonLines.length * 5 + 6;
  ...
  doc.text("DOCUMENTO COMPROBATÓRIO ANEXADO:", marginX + 6, currentY + 7);

  doc.setFont('courier', 'bold');
  doc.setFontSize(9.5);
  doc.setTextColor(15, 23, 42);
  doc.text(justification.fileName || "atestado_documento.pdf", marginX + 6, currentY + 15);

  currentY += 30;

  // Parecer da Secretaria / Status
  doc.setFont('times', 'bold');
  doc.setFontSize(10.5);
  doc.setTextColor(15, 23, 42);
  doc.text("PARECER E HOMOLOGAÇÃO DA SECRETARIA:", marginX, currentY);

  currentY += 5;
  const isApproved = justification.status === 'Aprovado';
  const isRejected = justification.status === 'Rejeitado';
  ...
  doc.setFont('times', 'bold');
  doc.setFontSize(10.5);
  if (isApproved) doc.setTextColor(22, 101, 52);
  else if (isRejected) doc.setTextColor(159, 18, 57);
  else doc.setTextColor(133, 77, 14);
  
  doc.text(`SITUAÇÃO REGIMENTAL: ${justification.status.toUpperCase()}`, marginX + 6, currentY + 8);

  doc.setFont('times', 'normal');
  doc.setFontSize(9);
  doc.setTextColor(51, 65, 85);
  const notes = justification.reviewerNotes || (isApproved ? 'Falta abonada regimentalmente pelo Secretário.' : 'Sob apreciação da Secretaria.');
  doc.text(`Observação: ${notes}`, marginX + 6, currentY + 15);
  if (justification.reviewedAt) {
    doc.text(`Data do Parecer: ${new Date(justification.reviewedAt).toLocaleDateString('pt-BR')}`, marginX + 6, currentY + 21);
  }
  ...
  doc.line(30, currentY + 18, 85, currentY + 18);
  doc.setFont('times', 'bold');
  doc.setFontSize(8.5);
  doc.setTextColor(15, 23, 42);
  doc.text('Obreiro Requerente', 57.5, currentY + 23, { align: 'center' });

  doc.line(125, currentY + 18, 180, currentY + 18);
  doc.text('Secretário da Loja', 152.5, currentY + 23, { align: 'center' });

  // Digital Hash Stamp
  const verificationHash = `CHANCELA DE ANEXO: JUST-${justification.id.toUpperCase()}-${Math.floor(100000 + Math.random() * 900000)}`;
  doc.setFontSize(7.5);
  doc.setFont('courier', 'normal');
  doc.setTextColor(148, 163, 184);
  doc.text(verificationHash, pageWidth / 2, 280, { align: 'center' });

  const outputName = justification.fileName || `justificativa_${justification.id}.pdf`;
  doc.save(outputName);
}

export function downloadJustificationAttachment(
  justification: Justification,
  member?: Member | { fullName: string; cim: string; degree: string },
  session?: Session
) {
  const fileName = justification.fileName || 'anexo_justificativa.pdf';
  if (justification.fileUrl && justification.fileUrl.trim() !== '') {
    try {
      const link = document.createElement('a');
      link.href = justification.fileUrl;
      ...
      console.warn('Direct download link failed, generating fallback PDF:', err);
    }
  }

  // Fallback if fileUrl is empty/missing
  generateJustificationDocumentPDF(justification, member, session);
}
